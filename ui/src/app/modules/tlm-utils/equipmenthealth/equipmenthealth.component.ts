...
@Component({
    selector: 'app-tlm-utils-equipmenthealth',
    templateUrl: './equipmenthealth.component.html',
    styleUrls: ['./equipmenthealth.component.scss'],
})
export class EquipmentHealthComponent extends BaseComponent implements OnInit, OnDestroy {
    ...
    {
        super('EquipmentHealthComponent', loggerService);
    }
    ...
    onChannelChange(event) {
        ...
        this.getEquipmentChannelData(this.selectedEquipment[0], this.selectedChannelName, '');
    }
        ...
        channelType: string) : void
    {
        ...
        this.equipmentHealthService.getChannelData(
            equipment,
            channelCode,
    ...
    getChannelListProcess(equipment: Equipment) {
        ...
        this.equipmentHealthService.verifyEquipmentExistsInRegistry(equipment).subscribe(
            eqInstance => {
    ...
    registerEquipment(equipment: Equipment) {
        this.equipmentHealthService.registerEquipmentInRegistry(equipment).subscribe(
            resp => {
                ...
                if (eqRegResp.id && eqRegResp.id.length > 0) {
                    ...
                    this.ToasterServiceInstace.ShowError('Error verifying Equipment Registry for' + equipment.materialNumber + ':' + equipment.sourceSystemRecordId);
                }
            },
            error => {
                this.WriteErrorLog('error in registering Equipment', error);
                this.ToasterServiceInstace.ShowError('Error registering Equipment in Registry for' + equipment.materialNumber + ':' + equipment.sourceSystemRecordId);
                return false;
            },
        );
    }
    getChannelList(equipment: Equipment) {
        ...
        this.equipmentHealthService.getChannelList(equipment).subscribe(
            channels => {
                ...
                if (this.channelCount && this.channelCount > 0) {
                    ...
                    this.ToasterServiceInstace.ShowError(' channel definitions for' + equipment.materialNumber + ':' + equipment.equipmentCode + ' not found');

                }
            },
            error => {
                this.WriteErrorLog('error in getChannelList', error);
                this.ToasterServiceInstace.ShowError(' channel definitions for' + equipment.materialNumber + ':' + equipment.equipmentCode + ' API error');
            },
        );
    }
    /**
